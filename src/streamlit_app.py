...
def main():
	...
	selected_erzeuger_names = st.sidebar.multiselect(
		"Erzeuger Typen wählen", erzeuger_options, default=["Photovoltaik"]
	)
	...
	with st.sidebar.form("add_datenpunkt"):
		...
		new_value = st.number_input(
			"Installierte Leistung (MW)", min_value=0.0, value=1000.0, step=100.0
		)
		...
		if submitted and dp_erzeuger_name:
			st.session_state["datenpunkte_list"].append(
				{"erzeuger": dp_erzeuger_name, "date": new_date, "value": new_value}
			)
		...
		with st.spinner("Lade Daten und berechne Prognose..."):
			...
			for name in selected_erzeuger_names:
				...
				erzeuger_dps = [
					ErzeugerDatenpunkt(
						art,
						datetime.datetime(d["date"].year, d["date"].month, d["date"].day),
						d["value"],
					)
				...
				if not erzeuger_dps:
					...
					erzeuger_dps = [
						ErzeugerDatenpunkt(art, datetime.datetime(2030, 1, 1), current_max)
					]
				...
				if combined_forecast is None:
					combined_forecast = prognose.df[["Datum von", name]].rename(
						columns={name: "Summe"}
					)
					combined_forecast_sim = prognose_sim.df[["Datum von", name]].rename(
						columns={name: "Summe"}
					)
				else:
	...
	if "last_multis_result" in st.session_state:
		...
		view_option = st.radio(
			"Ansicht wählen:", ("Summe aller Erzeuger", "Einzeln"), horizontal=True
		)
		...
		if view_option == "Summe aller Erzeuger":
			...
			chart_data[f"Mit Dürre {res['drought_intensity']} (MW)"] = res["combined_forecast_sim"][
				"Summe"
			].values
