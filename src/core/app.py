...
class App:
    ...
    def __init__(self) -> None:
        # Create the SMARD data access object once and reuse it
        self.smard: Smard = Smard()
        # Simple info log so we see that setup finished
    ...
    def run(self) -> None:
        ...
        pyplot.plot(prognose.df["Datum von"], prognose.df[ErzeugerArt.Photovoltaik], linewidth=0.8, label="Ohne Simulation", alpha=0.7)
        # Forecast after applying the drought simulation
        pyplot.plot(prognose_with_drought.df["Datum von"], prognose_with_drought.df[ErzeugerArt.Photovoltaik], linewidth=0.8, label="Mit Dürre (80%)", alpha=0.7)
